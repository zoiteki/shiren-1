...
import { getTheme } from '../../custom/config/codeBlockLanguages';
...
const CodeBlock = ({ children: exampleCode, ...props }) => {
  ...
  React.useEffect(() => {
    // Language support is now handled differently in prism-react-renderer v2
    updateView({
      data: Date.now()
    });
  }, []);
  ...
  if (props['react-live']) {
    ...
    return (
      <Highlight code={exampleCode} language={props.className.split("-")[1] ?? "javascript"} theme={theme}>
